 ...
 *
 * Vendored via new_client_implemntation, kept close to upstream.
 * Changes: TSX, perlin texture served from /public, trimmed unused props.
 */
import { Suspense, useEffect, useMemo, useRef } from "react";
import { Canvas, useFrame, useThree, useLoader } from "@react-three/fiber";
import * as THREE from "three";

export type OrbAgentState = "listening" | "talking" | "thinking" | null;
...
// Static gradient shown while the WebGL scene mounts and its texture loads —
// without it the <Canvas> renders nothing for a beat (or indefinitely, since
// useLoader suspends the r3f render tree and nothing outside it can resolve
// that promise).
function OrbFallback({ colors }: { colors: [string, string] }) {
  return (
    <div
      ...
      style={{
        background: `radial-gradient(circle at 35% 30%, ${colors[1]} 0%, ${colors[0]} 55%, ${colors[0]} 100%)`,
      }}
    />
  );
}
